...
# Business Creators are treated like any other Sales Tracker agent: each person (rep OR BC) has
# their own tracker tab, and the GM + Split % on that tab are ALREADY that person's own share of
# the deal (the tracker splits GM per agent at the agreed split). So a BC's GM and orders are read
# straight from rows where the BC is the `agent` — exactly like a rep — with no re-derivation.
